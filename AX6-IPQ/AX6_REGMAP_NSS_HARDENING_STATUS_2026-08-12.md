...
下表用于替代 `AX6_REPAIR_AND_FAULTS_ISSUE_LIST_2026-08-12.md` 中已经过期的
状态字段。这里的“源码已修”不等同于“新固件和实机已验证”。

| 旧 ID | 当前状态 | 说明 |
|---|---|---|
| P0-1 | 源码已修，待构建/实机 | `1002` 已按资源末端 clamp；新固件仍未生成，当前实机旧内核仍不得读取危险 regmap registers |
| P0-2 | 源码已修，待构建/实机 | APCS 改为保留驱动原上限的 clamp，并增加小资源保护 |
| P0-3 | 源码已修，待构建/冷启动 | PBUF 已改为精确回读、有限重试和失败传播；需冷启动十轮确认一次性分配行为 |
| P0-4 | 源码已修，待构建/冷启动 | `current_freq`、`auto_scale` 入口和两个平台消费端均已保护，不再只是“机制级采纳” |
| P0-5 | AX6 门禁有效，Track B 未完成 | 当前只允许 1/1/1/1 ring 和 identity-DMA；portable DMA 与 store 模型重构不属于本轮候选 |
| C-1 | 旧结论作废 | OpenClash 构建已记录实际提交、版本和 Meta core SHA256；按项目策略跟踪最新兼容版本，不回退锁定旧版 |
| C-2 | 已闭环 | 设备 manifest 与最终 rootfs 已安装包一致性门禁在上一成功构建中通过；本轮新产物仍需再次通过 |
| C-3 | 未修，低优先级 | 全量读取 `dev.nss` 触发调试日志不影响数据面；后续只能用定向提交处理，不能借机改频率逻辑 |
| C-4 | 已闭环 | `nss-check` 失败路径已附 verbose 快照 |
| C-5 | 未开始 | EDMA portable DMA 保持独立 Track B，不混入当前发布候选 |
| T-1/T-2/T-4 | 待授权/物理操作 | 冷启动、recovery 和回退验证不能由源码门禁代替 |
| T-3/T-5 | 环境阻塞 | 需要第二有线端点和专用 IoT 客户端 |
| T-7/T-8/T-9/T-10 | 未完全闭环 | PHY failure path、精确 netlink handler、72 小时增量窗和组件 provenance 仍需独立证据 |

另外，本轮新增 ECM 官方 signedness 回移未出现在旧清单中，现列为 P1 已修、
待构建/流量回归项目。

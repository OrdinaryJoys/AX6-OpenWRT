...
| `28580586309` | 成功 | P1 合入 `immortalwrt-nss/main` 后的 AX6 主锁 stock 构建完整通过,包含 `Compile firmware`、`Validate final rootfs contents`、artifact/kmod 上传和 Release 上传 |
...
| `28598719084` | `codex/ax6-p2c-qca-mcs-007-build-validation` | `codex/ax6-qca-mcs-007-candidate` / `35f03b6aed73779d5808031a0bff181c5830759d` | 进行中: clone、feeds、DIY、package download 已通过,当前在 `Compile firmware`;仍需确认最终 rootfs 检查 |
...
| P2c | qca-mcs no-nl80211 日志降噪 | 源码候选 `codex/ax6-qca-mcs-007-candidate`;AX6 验证分支 `codex/ax6-p2c-qca-mcs-007-build-validation` | 本地静态检查通过;构建 `28598719084` 已通过 clone、feeds、DIY、package download,正在编译 | 等 Actions 确认编译/rootfs;通过后可排入低风险候选 |

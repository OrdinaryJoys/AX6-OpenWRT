...
| 当前分支 HEAD | 本地 `a802d94`（成功构建代码候选 `193e5fbc276e`） |
| 成功构建源码 | `be691ad79541` |
| 新的测试/合并源码头 | `4e350435cd9e`（只新增 APCS fixture/test） |
| 源码 main | `56807d9661db` |
| 当前 lint | Actions `31315671414`，全部通过 |
| 当前 STOCK build | Actions `31315718824`，完整成功；新锁 `4e350435` 尚待重建 |
| 前一失败 build | Actions `31281669689`，packages HEAD 引入全局 Kconfig 递归依赖 |

运行 `31315718824` 已证明 `193e5fb + be691ad` 的完整编译、rootfs 和产物链。
独立下载后，sysupgrade/recovery/kmod 的 SHA256 全部通过，最终 rootfs 的 391 个
安装包与设备 manifest 完全一致，OpenClash core 和 AX6 管理脚本也与 BUILD-LOCK/
仓库源文件一致。新的 `4e350435` 只增加 APCS 静态测试，但锁定提交变化后仍需一次
新构建，不能把旧运行状态写成新头已验证。
...
| `4e350435` 测试锁完整构建 | 尚未运行 | 新 lint/build 和产物复核通过 |
